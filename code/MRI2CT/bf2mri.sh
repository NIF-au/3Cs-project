...
then
        ...
        then
	        parameters=$(zenity --forms --title="Add Parameters"  --separator="," --add-entry="Dimensionality" --entry-text "test" --add-entry="float" --add-entry="Interpolation" --add-entry="winsorize image intensities" --add-entry="Use histogram matching" --add-entry="Num_transform" --add-entry="Convergence" --add-entry="Metric" --add-entry="Shrink Factors" --add-entry="Smoothing sigmas")
                ...
                antsRegistration --dimensionality ${par[1]} --float ${par[2]} --output [$out_dir/$TMN,$out_dir/RIN] --interpolation ${par[3]} --winsorize-image-intensities [${par[4]}, ${par[5]}] --use-histogram-matching ${par[6]} --initial-moving-transform [$fixed, $moving, 1] --transform $method[${par[6]}, ${par[7]}, ${par[8]}] --metric MI[$fixed,$moving,${par[9]},${par[10]}] --convergence [${par[11]}x${par[12]}x${par[13]}x${par[14]},${par[15]},${par[16]}] --shrink-factors ${par[17]}x${par[18]}x${par[19]}x${par[20]} --smoothing-sigmas ${par[21]}x${par[22]}x${par[22]}x${par[23]}vox --verbose
